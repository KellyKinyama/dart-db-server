...
class Parser {
  ...
  SelectStatement _parseSelectStatement() {
    ...
    // Fix: Use a boolean flag for the DISTINCT keyword, which is what the AST expects.
  ...
  List<dynamic> _parseSelectClause() {
    // Bug Fix: The original code used a Set (<dynamic>{}), which can't have duplicate items.
    // Using a List (<dynamic>[]) is more appropriate for a column list.
  ...
  TableExpression _parseFromClause() {
    ...
    // A more robust way to parse joins would be to check for the JOIN keyword first,
    // and then handle the optional type keywords (LEFT, RIGHT, etc.).
    // This implementation is a bit fragile if the user types something unexpected.
  ...
  HavingClause _parseHavingClause() {
    ...
    return HavingClause(expression: _parseExpression());
  }
  ...
  LimitClause _parseLimitClause() {
    ...
    if (token.type == TokenType.NUMBER) {
      _consume();
      // Parse the number as a double first to handle potential floats, then cast to int if needed.
  ...
  /// Parses a comparison expression or a BETWEEN/IN/LIKE/IS clause.
  Expression _parseComparison() {
    ...
    if (opToken.type == TokenType.KEYWORD_BETWEEN) {
      _consume();
      final lowerBound = _parsePrimaryExpression();
      _expect(TokenType.KEYWORD_AND,
          'Expected AND after lower bound in BETWEEN clause');
      final upperBound = _parsePrimaryExpression();
    ...
    if (opToken.type == TokenType.KEYWORD_IN) {
      ...
      if (_peek().type != TokenType.RIGHT_PAREN) {
        list.add(_parsePrimaryExpression());
        while (_peek().type == TokenType.COMMA) {
          _consume();
          list.add(_parsePrimaryExpression());
        }
...
void main() {
  final sqlQueries = [
    // SELECT statements
    "SELECT DISTINCT name, email FROM users WHERE last_login IS NOT NULL;",
    "SELECT category, COUNT(*) FROM products GROUP BY category HAVING COUNT(*) > 10;",
    "SELECT name FROM employees WHERE department LIKE 'Sales%' ORDER BY name ASC;",
    "SELECT MIN(price), MAX(price), AVG(price) FROM products WHERE category = 'electronics';",
    "SELECT id, name FROM users WHERE id IN (1, 2, 5, 8);",
    "SELECT * FROM orders INNER JOIN customers ON orders.customer_id = customers.id;",
    "SELECT name FROM products WHERE price BETWEEN 10.5 AND 99.99;",
    ...
    "UPDATE users SET status = 'active' WHERE name LIKE 'A%';",
    "INSERT INTO users (name, email) VALUES ('John Doe', 'john.doe@example.com');",
    "DELETE FROM users WHERE last_login < 1672531200;",
  ];
