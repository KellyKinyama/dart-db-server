...
enum TokenType {
  ...
  KEYWORD_RIGHT,
  KEYWORD_OUTER,
  KEYWORD_MAX, // Added for MAX function
  KEYWORD_BETWEEN, // Added for BETWEEN clause
  KEYWORD_IN, // Added for IN clause
  KEYWORD_ASCENDING, // Added for ASCENDING keyword
  KEYWORD_DESCENDING, // Added for DESCENDING keyword
...
TokenType _getTokenType(String value) {
  switch (value.toUpperCase()) {
    ...
    case 'LEFT':
      return TokenType.KEYWORD_LEFT;
    case 'RIGHT':
      return TokenType.KEYWORD_RIGHT;
    case 'OUTER':
      return TokenType.KEYWORD_OUTER;
    case 'MAX':
      return TokenType.KEYWORD_MAX;
    case 'BETWEEN':
      return TokenType.KEYWORD_BETWEEN;
    case 'IN':
      return TokenType.KEYWORD_IN;
    case 'ASCENDING':
      return TokenType.KEYWORD_ASCENDING;
    case 'DESCENDING':
      return TokenType.KEYWORD_DESCENDING;
    default:
...
class Parser {
  ...
  TableExpression _parseFromClause() {
    ...
    // Check for join type keywords before the JOIN keyword itself
  ...
  /// Parses a comparison expression or a BETWEEN/IN clause.
  Expression _parseComparison() {
    ...
    // Check for a standard comparison operator
    if ([
      TokenType.EQUALS,
  ...
  Expression _parsePrimaryExpression() {
    ...
    if (token.type == TokenType.IDENTIFIER ||
        token.type == TokenType.KEYWORD_MAX) {
      _consume();
...
void main() {
  final sqlQueries = [
    ...
    "SELECT MAX(price) FROM products WHERE category IN ('electronics', 'books') AND stock_level BETWEEN 10 AND 50;",
    "SELECT category, COUNT(*) FROM products GROUP BY category ORDER BY COUNT(*) DESCENDING LIMIT 5;",
