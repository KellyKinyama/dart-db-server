...
void main() {
  ...
  test('GROUP_CONCAT with ORDER BY sorts contributions', () async {
    ...
    try {
      ...
      await db.execute("INSERT INTO t VALUES ('c',3),('a',1),('b',2)");
      final r =
          await db.execute("SELECT GROUP_CONCAT(name, ',' ORDER BY n) FROM t");
      expect(r.rows.first[0], 'a,b,c');
      final r2 = await db
          .execute("SELECT GROUP_CONCAT(name, ',' ORDER BY n DESC) FROM t");
  ...
  test('JSON_GROUP_ARRAY with ORDER BY', () async {
    ...
    try {
      ...
      final r =
          await db.execute('SELECT JSON_GROUP_ARRAY(n ORDER BY n) FROM t');
