  ...
  group('UPDATE ... FROM', () {
    test('joins target with other table to update values', () async {
      ...
      try {
        ...
        await db.execute('INSERT INTO patch VALUES (1, 100), (3, 300)');
        await db.execute(
    ...
    test('FROM with alias', () async {
      ...
      try {
        ...
        await db
            .execute('UPDATE t SET x = pp.v FROM p AS pp WHERE t.id = pp.id');
    ...
    test('rows without a join match are left untouched', () async {
      ...
      try {
        ...
        await db.execute('UPDATE t SET x = p.v FROM p WHERE t.id = p.id');
        final r = await db.execute('SELECT id, x FROM t ORDER BY id');
        expect(r.rows, [
          [1, 99],
          [2, 2],
