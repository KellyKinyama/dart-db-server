...
void main() {
  group('rtree planner', () {
    test('point-in-box query is served by the R-tree index', () async {
      ...
      try {
        ...
        final r = await db.execute(
            'SELECT id FROM shapes WHERE x0 <= 5 AND x1 >= 5 '
    ...
    test('large rtree: index returns the same rows as a brute scan',
        () async {
      final db = await Database.open();
      try {
        await db.execute(
            'CREATE VIRTUAL TABLE s USING rtree(id, x0, x1, y0, y1)');
        ...
        for (var i = 0; i < 300; i++) {
          ...
          await db.execute(
              'INSERT INTO s VALUES ($i, ${x.toStringAsFixed(4)}, '
    ...
    test('rtree cache is invalidated after INSERT', () async {
      ...
      try {
        await db.execute(
            'CREATE VIRTUAL TABLE s USING rtree(id, x0, x1)');
        ...
        final r = await db.execute(
            'SELECT id FROM s WHERE x0 <= 10.5 AND x1 >= 10.5');
        expect(r.rows, [
          [2]
        ]);
      } finally {
